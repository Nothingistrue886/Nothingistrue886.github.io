...
- 使用 java.util.concurrent.CopyOnWriteArrayList 并发包下的CopyOnWriteArrayList 代替 ArrayList，该容器内部会对 add、remove、clear 等方法进行加锁操作，是一个线程安全的 List。其实现原理在于，每次add、remove 等操作都是重新创建一个新的数组，再把引用指向新的数组。 如下源码 ：
